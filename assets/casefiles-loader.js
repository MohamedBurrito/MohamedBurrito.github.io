...
(function () {
  ...
  function renderCard(entry) {
    ...
    const titleInner = entry.published
      ? `<a href="${escapeHtml(entry.file)}" style="color:inherit;text-decoration:none;border-bottom:1px dotted var(--ink-soft);">${escapeHtml(entry.title)}</a>`
      : `${escapeHtml(entry.title)}`;

    const redactedStrip = entry.published ? '' : `<div class="redacted-strip"></div>`;

    return `
    <div class="file" data-view="${entry.view}" data-diff="${entry.difficulty}" data-slug="${entry.slug}">
